...
export class UserMessagesPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.newMessageSub = this.socketEventsService.listenToObservableEventStream(EVENT_TYPES.NEW_MESSAGE)
      .subscribe((event: any) => {
  ...
  getMessagings() {
    ...
    this.usersService.get_user_messagings(this.you!.id, undefined, true).subscribe({
      next: (response: any) => {
  ...
  getMessages() {
    ...
    this.usersService.get_user_messages(
      this.you!.id,
      other_user.id,
      min_id
    ).subscribe({
