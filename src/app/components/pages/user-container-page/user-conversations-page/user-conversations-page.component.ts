...
import { IConversation, IConversationMember, IConversationMessage } from 'src/app/interfaces/conversation.interface';
...
@Component({
  ...
  templateUrl: './user-conversations-page.component.html',
  styleUrls: ['./user-conversations-page.component.scss']
})
export class UserConversationsPageComponent implements OnInit, OnDestroy {
  ...
  currentConversationSelected: IConversation | null;
  loading = false;
  ...
  conversations_map: {[key:string]: IConversation} = {};
  conversations_list: IConversation[] = [];
  conversation_members_list: IConversationMember[] = [];
  conversation_messages_list: IConversationMessage[] = [];
  conversations_list_end = true;
  conversation_messages_list_end = true;
  
  // usersTyping: any[] = [];
  usersTypingMap: {[key:string]: IUser[]} = {};
  ...
  ngOnInit(): void {
    ...
    this.newConversationSub = this.socketEventsService.listenToObservableEventStream(EVENT_TYPES.CONVERSATION_MEMBER_ADDED).subscribe((event: any) => {
      this.handleMemberAddedEvent(event);
    });
    this.removedSub = this.socketEventsService.listenToObservableEventStream(EVENT_TYPES.CONVERSATION_MEMBER_REMOVED).subscribe((event: any) => {
  ...
  setConversation(conversation: any) {
    ...
    this.conversationsService.update_conversation_last_opened(this.you.id, this.currentConversationSelected!.id)
      .subscribe((response) => {
  ...
  addListeners(conversation_id: number) {
    ...
    this.socketTypingEmittersMap[conversation_id] = this.socketEventsService.listenSocketCustom(
      EVENT_TYPES.CONVERSATION_MESSAGE_TYPING,
      (event) => {
        console.log('member is typing...', event);
        if (this.you.id !== event.user.id || event.conversation_id !== this.currentConversationSelected?.id) {
    ...
    this.socketTypingStoppedEmittersMap[conversation_id] = this.socketEventsService.listenSocketCustom(
      EVENT_TYPES.CONVERSATION_MESSAGE_TYPING_STOPPED,
      (event) => {
        console.log('member stopped typing...', event);
  ...
  removeListeners(conversation_id: number, eventType?: EVENT_TYPES)  {
    ...
    if (this.socketTypingEmittersMap[conversation_id]) {
      this.socketTypingEmittersMap[conversation_id].off(`${EVENT_TYPES.CONVERSATION_MESSAGE_TYPING}:conversation-${conversation_id}`);
    }
    if (this.socketTypingStoppedEmittersMap[conversation_id]) {
      this.socketTypingStoppedEmittersMap[conversation_id].off(`${EVENT_TYPES.CONVERSATION_MESSAGE_TYPING_STOPPED}:conversation-${conversation_id}`);
    }
    if (this.socketMemberAddedEmittersMap[conversation_id]) {
      this.socketMemberAddedEmittersMap[conversation_id].off(`${EVENT_TYPES.CONVERSATION_MEMBER_ADDED}:conversation-${conversation_id}`);
    }
    if (this.socketMemberRemovedEmittersMap[conversation_id]) {
      this.socketMemberRemovedEmittersMap[conversation_id].off(`${EVENT_TYPES.CONVERSATION_MEMBER_REMOVED}:conversation-${conversation_id}`);
    }
    if (this.socketNewMessageEmittersMap[conversation_id]) {
      this.socketNewMessageEmittersMap[conversation_id].off(`${EVENT_TYPES.NEW_CONVERSATION_MESSAGE}:conversation-${conversation_id}`);
    }
    if (this.socketConversationDeletedEmittersMap[conversation_id]) {
      this.socketNewMessageEmittersMap[conversation_id].off(`${EVENT_TYPES.CONVERSATION_DELETED}:conversation-${conversation_id}`);
    }
  }
  ...
  handleMessageEvent(event: any) {
    ...
    if (isCurrentSelectedMessaging) {
      ...
      if (conversation) {
        ...
        if (conversation.hasOwnProperty('unseen_messages_count') && typeof(conversation.unseen_messages_count) === 'number') {
  ...
  createConversation(conversationFormElm: HTMLFormElement) {
    ...
    this.conversationsService.create_conversation(
      ...
      next: (response: any) => {
        this.loading = false;
        this.conversations_list.unshift(response.data.conversation);
        this.conversations_map[response.data.conversation.id] = response.data.conversation;
        this.usersTypingMap[response.data.conversation.id] = [];
        this.addListeners(response.data.conversation.id)
  ...
  editConversation(conversationFormElm: HTMLFormElement) {
    ...
    this.conversationsService.update_conversation(
      ...
      next: (response: any) => {
        this.currentConversationSelected!.title = response.data.conversation.title;
        this.currentConversationSelected!.icon_link = response.data.conversation.icon_link;
        this.currentConversationSelected!.icon_id = response.data.conversation.icon_id;
    ...
    this.conversationsService.add_conversation_member(
      ...
      next: (response: any) => {
        ...
        if (this.conversations_map[this.currentConversationSelected!.id].hasOwnProperty('members_count')) {
          const id = this.currentConversationSelected!.id;
          const conversation = this.conversations_map[id];
          (<any> conversation).members_count++;
        }
        this.alertService.showSuccessMessage(response.message);
      },
      error: (error: HttpErrorResponse) => {
